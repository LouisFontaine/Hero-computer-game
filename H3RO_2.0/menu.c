...
/**Fonction qui va lire les inputs et qui va déplacer le curseur de start à quit en
fonction du choix de l'utilisateur et qui va se charger de lancer le jeu quand l'utilisateur le décide*/
void renewStartMenu(void)
{
    //Quand on appuie sur flêche du bas
    if(input.down == 1)
    {
        //Si choice égal à O (start), on le met sur 1 (quit)
    ...
    if(input.enter)
    {
        if(game.choice == 0)
        {
            ...
            //On réinitialise les variables du jeu pour commencer une nouvelle partie
            game.lives = 3;
            game.etoiles = 0;
            game.onMenu = 0;
        }
...
/**Fonction qui va lire les inputs pour pouvoir relancer le jeu lorsque l'utilisateur
rappuie sur enter*/
...
void renewEndMenu(void)
{
    ...
    if(input.enter)
    {
        //On ne change pas l'état onMenu mais on change le type de menu
        game.menuType = START;
        input.enter = 0;
    }
}
