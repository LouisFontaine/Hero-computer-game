...
SDL_Surface *loadImage(char *name)
{
    ...
    //Di on arrive pas à la charger on quitte le programme et on affiche dans la commande le problème
    if (temp == NULL)
    {
...
void paintHud(void)
{
    int ysource;
    int xsource;
    //Cré un variable de type text pour l'affichage du nombre de vie et de coeur
    char text[200];
    ...
    //Affichage des lives en bas à droite de l'écran
    paintImage(game.HUD_vie, 900, 600);

    //On met le nombre de vie dans la chaine de carractere
    sprintf(text, "%d", game.lives);

    //Puis on reutilise la fonction d'affichage de string créée précedement
    paintString(text, 970, 610, font);

    //Affichage du nombre d'étoile
